...
description: >
  ...
  publish, QR admin verification handling, result archive, and scheduling. Use whenever the
  user wants reproducible 公众号自动发文 with API and/or browser control, while keeping all
  secrets outside the skill package.
---
...
| 文件 | 用途 |
|------|------|
| `references/environment-and-config.md` | 环境、密钥占位、代理、Chrome 调试前提 |
...
- **通道 B**：编辑器「发表」→ 处理创作来源声明 → 群发通知确认 →「继续发表」→ **管理员扫码验证**（常见）→ 在「发表记录」核对「已发表」  
...
3. **正式发表常弹「微信验证」二维码**：必须管理员/运营者扫码，Agent 无法代替。  
...
7. **生图与微信 API 代理策略相反**：生图可代理，微信 API 须直连。  
